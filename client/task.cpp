    ...
    ui(new Ui::Task)
{
    ...
    Socket::GetInstance()->sk.write("names&"); // Возможность вывода задания через поля ввода текста
...
{
    QString cmd = "task_template&" + name;
...
{
    QString cmd = "answer&" + ui->plainTextEditAnswer->toPlainText()+"&";
    Socket::GetInstance()->sk.write(cmd.toUtf8());
    Socket::GetInstance()->sk.waitForReadyRead(3000);
