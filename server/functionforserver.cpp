...
QByteArray auth(QString log, QString pass)
{
    ...
    if (DBase::getInstance()->doSQLQuery(query).size() > 0) {
        DBase::getInstance()->doSQLQuery("UPDATE Users SET in_sys = 1 WHERE log = \"" + log + "\"");
        return "auth+";
    }
    else {
        return "auth-";
...
QByteArray parsing(QString data_from_client)
{
    ...
    // Добавляем оператор return в конце функции
    return QByteArray(); // Возвращает пустой QByteArray, или другое значение по вашему выбору
}
